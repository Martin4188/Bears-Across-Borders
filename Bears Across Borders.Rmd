...
The largest share of the bias is caused by the size of the bears territories.
...
For $\lambda$ we use values 2, 3 and 4 and for $\sigma$ we use 0.1, 0.2 and 0.3.
...
NaiveSimulation %>%
  ...
  select(-PopEstimate) %>%
  pivot_longer(SE:relativeBias, names_to = "metric")  %>%
  mutate(lambda = lambda %>% as.factor()) %>%
  ggplot(aes(x = sigma, y = value, color = lambda)) + geom_line()  + geom_point() + 
  facet_wrap(~metric, scales = "free_y") +
  labs(title = "Figure 1: Population estimate mean bias and standard error. (small sigma)", x = "Sigma", y = "")
...
NaiveSimulation %>%
  ...
  ggplot(aes(x = sigma, y = value, color = lambda)) + geom_line()  + geom_point() + 
  facet_wrap(~metric, scales = "free_y") +
  labs(title = "Figure 1: Population estimate mean bias and standard error.", x = "Sigma", y = "")
```
...
NaiveSimulation %>%
  ...
  mutate(bias = MLE - lambda,
         SQRTMSE = sqrt((MLE - lambda)^2)) %>%
  group_by(lambda, sigma) %>%
  summarise(SE = sd(MLE),
            across(where(is.numeric), mean)) %>%
  pivot_longer(c(SE, bias), names_to = "metric") %>%
  mutate(lambda = lambda %>% as.factor()) %>%
  ggplot(aes(x = sigma, y = value, color = lambda)) + geom_line()  + geom_point() + 
  facet_wrap(~metric, scales = "free_y") +
  labs(title = "MLE bias and standard error.", x = "Sigma", y = "")
```
